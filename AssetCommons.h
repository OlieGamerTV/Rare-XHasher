...
static unsigned long long getLongLongFromHashArray(int initialPosition) {
	...
	val = val + (initialVal3 << 16);
	val = val + (initialVal4 << 24);
	val = val + (initialVal5 << 32);
	val = val + (initialVal6 << 40);
	val = val + (initialVal7 << 48);
	val = val + (initialVal8 << 48);
...
static unsigned int assetIdGetHash_BaseExt(char* str, bool verbose = false) {
	...
	while (currChar != '\0') {
		...
		if (verbose) {
			printf("Current Hash -> %016I64x | ", hash);
...
static unsigned int assetIdGetHash_Typed(char* str, bool verbose = false) {
	...
	if (strstr(str, "XACT_CUE_") != 0) {
		curVal = LowerToUpper(str);
		curVal = curVal + 10;
	}
...
/// <summary>
/// Generates a four-byte hash from the provided string.
/// <para>The ones generated by this function are valid for Perfect Dark Zero.</para>
/// </summary>
/// <param name="str"></param>
/// <returns>An unsigned integer containing the hash.</returns>
static unsigned int assetIdGetHash_Dark(char* str, bool verbose = false) {
	...
	if (*str == '\0') {
		printf("A blank string has been used for assetIdGetHash_Dark(), this won't do anything.\n");
		return 0;
	}
...
/// <summary>
/// Generates a four-byte hash from the provided string.
/// <para>The ones generated by this function are valid for Kameo: Elements of Power.</para>
/// </summary>
/// <param name="str"></param>
/// <returns>An unsigned integer containing the hash.</returns>
static unsigned int assetIdGetHash_Dolphin(char* str, bool verbose = false) {
	...
	if (*str == '\0') {
		printf("A blank string has been used for assetIdGetHash_Dolphin(), this won't do anything.\n");
		return 0;
	}
	...
	while (currChar != '\0') {
		...
		// More or less the same as the one used for Ghoulies and Viva Pinata.
		// The only actual difference is we don't mask the current character here.
		hash = hash * 0x10 + currChar;
